...
Class wp_plugin_fancy_gallery {
  ...
  var $version = '1.0.10'; // Current release number
  ...
  function __construct(){
    ...
    Else {
      ...
      WP_Enqueue_Script('jquery.easing', $this->base_url . '/jquery.easing.1.3.js', Array('jquery'), '1.3', ($this->get_option('script_position') != 'header') );
      WP_Enqueue_Script('jquery.mousewheel', $this->base_url . '/jquery.mousewheel-3.0.4.pack.js', Array('jquery'), '3.0.4', ($this->get_option('script_position') != 'header') );
      WP_Enqueue_Script('fancybox', $this->base_url . '/fancybox/jquery.fancybox-1.3.4.pack.js', Array('jquery'), '1.3.4', ($this->get_option('script_position') != 'header') );
  ...
  function Render_Gallery ($template_file){
    ...
    // Strip Line Breaks (They would break the excerpts!)
    $code = Str_Replace("\n", ' ', $code);
  ...
  function ShortCode_Gallery ($attributes = Array()){
    $attributes = $this->Generate_Gallery_Attributes($attributes);
  	...
  	// get attachments
    If (Empty($attributes['include'])){ // this gallery uses the post attachments
      #If (!Current_User_Can('read_post', $attributes['post_parent'])) return;
      #If (Get_Post_Status($attributes['post_parent']) != 'publish') return;
